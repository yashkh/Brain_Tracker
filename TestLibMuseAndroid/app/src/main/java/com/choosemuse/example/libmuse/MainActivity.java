...
public class MainActivity extends Activity implements OnClickListener {
    ...
    private ArrayList<String> eeg_TP9Values = new ArrayList<String>();
    private ArrayList<String> eeg_TP10Values = new ArrayList<String>();
    private ArrayList<String> eeg_AF7Values = new ArrayList<String>();
    private ArrayList<String> eeg_AF8Values = new ArrayList<String>();

    private ArrayList<ArrayList<Double>> alphaValues = new ArrayList<ArrayList<Double>>();
    private ArrayList<ArrayList<Double>> betaValues = new ArrayList<ArrayList<Double>>();
    private ArrayList<ArrayList<Double>> deltaValues = new ArrayList<ArrayList<Double>>();
    private ArrayList<ArrayList<Double>> gammaValues = new ArrayList<ArrayList<Double>>();
    private ArrayList<ArrayList<Double>> rValues = new ArrayList<ArrayList<Double>>();
    private ArrayList<String> currentTimeStamp = new ArrayList<String>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            mClient = new MobileServiceClient("https://testlibmuseandroid.azurewebsites.net", this);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.refresh) {
            ...
            if (availableMuses.size() < 1 || musesSpinner.getAdapter().getCount() < 1) {
                ...
                System.out.println("SEGSERGSERGajwhefjawegfjhawefjhagwejhfaAERGERGAERGAERGAER");
                ...
                System.out.println("SEGSERGSERGajwhefjawegfjhawefjhagwejhfaAERGERGAERGAERGAER");
                System.out.println(alphaValues.size());
                for (ArrayList aList : alphaValues) {
                    for (Object aV : aList) {
                        System.out.println(Double.parseDouble(aV.toString()));
                    }
                    System.out.println();
                }
            ...
            */
            ...
            alphaValues = new ArrayList<ArrayList<Double>>();
            betaValues = new ArrayList<ArrayList<Double>>();
            deltaValues = new ArrayList<ArrayList<Double>>();
            gammaValues = new ArrayList<ArrayList<Double>>();
            rValues = new ArrayList<ArrayList<Double>>();
            currentTimeStamp = new ArrayList<String>();


        } else if (v.getId() == R.id.send_data) {
            Toast.makeText(getApplicationContext(),
                    "sdfndfjdf", Toast.LENGTH_LONG).show();


                //System.out.print("dsadSA");
            //ArrayList<Double> tempA = new ArrayList<Double>();
            //tempA.add(1.0); tempA.add(2.0); tempA.add(5.0); tempA.add(4.0);
            //ArrayList<ArrayList<Double>> a = new ArrayList<>();
            //a.add(tempA);
            //a.add(tempA);

            checkLogin(currentTimeStamp,alphaValues, betaValues, gammaValues, deltaValues);
            }
        }

    private void checkLogin(final ArrayList<String> time, final ArrayList<ArrayList<Double>> alpha,final ArrayList<ArrayList<Double>> beta,final ArrayList<ArrayList<Double>> gamma,final ArrayList<ArrayList<Double>> delta) {
        ...
        StringRequest strReq = new StringRequest(Request.Method.POST,
                "http://icici-trinity-team-ai.comxa.com/android_login_api/login.php", new Response.Listener<String>() {
            ...
            protected Map<String, String> getParams() {
                ...
                for (int i = 0; i < alpha.size(); i++)
                    sa += alpha.get(i).toString() + "@";
                System.out.print(sa);
                sa.substring(0,sa.length() -2);

                String sb = "";
                for (int i = 0; i < beta.size(); i++)
                    sb += beta.get(i).toString() + "@";
                System.out.print(sb);
                sb.substring(0,sb.length() -2);

                String sg = "";
                for (int i = 0; i < gamma.size(); i++)
                    sg += gamma.get(i).toString() + "@";
                System.out.print(sg);
                sg.substring(0,sg.length() -2);

                String sd = "";
                for (int i = 0; i < delta.size(); i++)
                    sd += delta.get(i).toString() + "@";
                System.out.print(sd);
                sd.substring(0,sd.length() -2);

                String tt = "";
                for (int i = 0; i < time.size(); i++)
                    tt += time.get(i).toString() + "@";
                System.out.print(tt);
                tt.substring(0,tt.length() -2);
    ...
    public void receiveMuseDataPacket(final MuseDataPacket p, final Muse muse) {
        ...
        //System.out.println("kjbkjakajdfjk" + p);
        switch (p.packetType()) {
            case EEG:
            ...
            case ALPHA_RELATIVE:
                ...
                currentTimeStamp.add(ts);
                System.out.println( currentTimeStamp.size());
                getEegChannelValues(alphaBuffer,p);
                ArrayList<Double> temp = new ArrayList<Double>();
                if(Double.isNaN(alphaBuffer[0]))
                    alphaBuffer[0] = 0.0;
                if(Double.isNaN(alphaBuffer[1]))
                    alphaBuffer[1] = 0.0;
                if(Double.isNaN(alphaBuffer[2]))
                    alphaBuffer[2] = 0.0;
                if(Double.isNaN(alphaBuffer[3]))
                    alphaBuffer[3] = 0.0;
                temp.add(alphaBuffer[0]);
                temp.add(alphaBuffer[1]);
                temp.add(alphaBuffer[2]);
                temp.add(alphaBuffer[3]);
                alphaValues.add(temp);
                //System.out.println("LALALALLA"+alphaValues.get(alphaValues.size()-1).get(2));
                alphaStale = true;
                break;
            case BETA_RELATIVE:
                assert(alphaBuffer.length >= n);
                ...
                if(Double.isNaN(alphaBuffer[3]))
                    alphaBuffer[3] = 0.0;
                temp1.add(alphaBuffer[0]);
                temp1.add(alphaBuffer[1]);
                temp1.add(alphaBuffer[2]);
                temp1.add(alphaBuffer[3]);
                betaValues.add(temp1);
                alphaStale = true;
                break;
            case GAMMA_RELATIVE:
                assert(alphaBuffer.length >= n);
                ...
                if(Double.isNaN(alphaBuffer[3]))
                    alphaBuffer[3] = 0.0;
                temp2.add(alphaBuffer[0]);
                temp2.add(alphaBuffer[1]);
                temp2.add(alphaBuffer[2]);
                temp2.add(alphaBuffer[3]);
                gammaValues.add(temp2);
                alphaStale = true;
                break;
            ...
            case DELTA_RELATIVE:
                ...
                if(Double.isNaN(alphaBuffer[3]))
                    alphaBuffer[3] = 0.0;
                temp3.add(alphaBuffer[0]);
                temp3.add(alphaBuffer[1]);
                temp3.add(alphaBuffer[2]);
                temp3.add(alphaBuffer[3]);
                deltaValues.add(temp3);
                alphaStale = true;
                break;
    ...
    private void updateEeg() {
        ...
        eeg_TP9Values.add(String.format("%6.2f", eegBuffer[0]));
        fp1.setText(String.format("%6.2f", eegBuffer[1]));
        eeg_AF7Values.add(String.format("%6.2f", eegBuffer[1]));
        fp2.setText(String.format("%6.2f", eegBuffer[2]));
        eeg_AF8Values.add(String.format("%6.2f", eegBuffer[2]));
        tp10.setText(String.format("%6.2f", eegBuffer[3]));
        eeg_TP10Values.add(String.format("%6.2f", eegBuffer[3]));
    }
...
ToDoItem item = new ToDoItem();
            //item.timeStampData = "abcds";
            int length = alphaValues.size();
            System.out.print(length);
            for (int i=0; i<length; i++){
                System.out.println(alphaValues.get(i));
                item.timeStamp = currentTimeStamp.get(i);
                String sa = "";
                String sb = "";
                String sg = "";
                String sd = "";
                for (int j =0 ; j<4; j++){
                    sa += alphaValues.get(j) +",";
                    sb += betaValues.get(j) +",";
                    sg += gammaValues.get(j) +",";
                    sd += deltaValues.get(j) +",";
                }
            ...
            mClient.getTable(ToDoItem.class).insert(item, new TableOperationCallback<ToDoItem>() {
                ...
                public void onCompleted(ToDoItem entity, Exception exception, ServiceFilterResponse response) {
                    if (exception == null)
                        ...
                        Toast.makeText(getApplicationContext(), "dydtdy", Toast.LENGTH_SHORT).show();
                    }
                }
            });
 */
...
/*
            int length = alphaValues.size();
            for (int i=0;i<length; i++){
                item.timeStampData = currentTimeStamp.get(i);
                for(int j =0; j<4; j++){
                    item.alpha.set(i, ) =  alphaValues.get(i).get(j);
                }
                writer.print("\t");
                for(int j =0; j<4; j++){
                    writer.print(betaValues.get(i).get(j)+",");
                }
                writer.print("\t");
                for(int j =0; j<4; j++){
                    writer.print(gammaValues.get(i).get(j)+",");
                }
                writer.print("\t");
                for(int j =0; j<4; j++){
                    writer.print(deltaValues.get(i).get(j)+",");
                }
                writer.print("\t");
            ...
            System.out.println("hihfkjahdjfahks");
            File[] allfiles = ContextCompat.getExternalFilesDirs(this, Environment.DIRECTORY_DCIM);

            int flag = 0;
            String[] parts;
            String directoryPath = "";
            for (int i = 0; i < allfiles.length; i++) {
                String sdpath = allfiles[i].getAbsolutePath();
                parts = sdpath.split("/");
                directoryPath = "/" + parts[1] + "/" + parts[2] + "/" + "abcd.txt";
            ...
            if (textFile.exists()) {
                ...
                try {
                    ...
                    System.out.println("kjfakwjefakjwe");
                    bw.write("hello");
                    System.out.println("akwefkawefkkejf");
                } catch (IOException e) {
            ...
            String url = "jdbc:sqlserver://lahacksserver.database.windows.net:1433;database=LAHacks;user=serveradmin@lahacksserver;password=lahacks@2017;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;";
            ...
            try {
                ...
                String selectSql = "SELECT * from dbo.fileData";
            ...
            try{
                ...
                int length = alphaValues.size();
                for (int i=0;i<length; i++){
                    writer.print(currentTimeStamp.get(i) + "\t");
                    for(int j =0; j<4; j++){
                        writer.print(alphaValues.get(i).get(j)+",");
                    }
                    writer.print("\t");
                    for(int j =0; j<4; j++){
                        writer.print(betaValues.get(i).get(j)+",");
